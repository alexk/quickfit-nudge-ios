...
final class StreakManager: ObservableObject {
    ...
    func updateStreak(for completion: WorkoutCompletion) {
        ...
        // Update family streak if applicable
        if completion.withKid,
           let familyIndex = streaks.firstIndex(where: { $0.type == .familyFriendly }) {
            updateFamilyStreak(at: familyIndex, completionDate: completionDate)
        }
    ...
    private func updateFamilyStreak(at index: Int, completionDate: Date) {
        let lastActivity = streaks[index].lastActivityDate
        ...
        if !calendar.isDate(lastActivity, inSameDayAs: completionDate) {
            ...
            if daysDiff <= 7 { // Allow weekly family activities
                streaks[index].incrementStreak()
            } else {
                streaks[index].resetStreak()
                streaks[index].incrementStreak()

...
struct QuickFitNudgeApp: App {
    ...
    init() {
        ...
        // Configure UniversalAnalytics
        let config = UniversalAnalyticsConfiguration(
            apiKey: ProcessInfo.processInfo.environment["UNIVERSAL_ANALYTICS_API_KEY"] ?? "",
            appName: "FitDad Nudge",
            appVersion: Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String ?? "1.0",
            debugMode: false
        )
        UniversalAnalytics.shared.configure(config)
        
        // Initialize AnalyticsManager (which will now forward to UniversalAnalytics)
